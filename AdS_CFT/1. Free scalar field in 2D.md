...
$$
Often a space direction is compactified from $\mathbb{R}$ to $\mathbb{S}^1$ , so that $x^1+2\pi=x^1$. Then $z=e^\xi$ maps the compactified spacetime of cylindrical shape to a complex plane. One defines
$$
...
where
$$
d^2\xi=\frac{i}{2}d\xi\wedge d\bar\xi,~d^2z=\frac{i}{2}dz\wedge d\bar z
$$
...
$$
Such a transformation preserves angles between vectors.
...
$$
For closed strings, periodic boundary conditions, $\hat X(x^0,x^1)=\hat X(x^0,x^1+2\pi)$ , are automatically obeyed by $\hat X(z,\bar z)$, and one can write a Laurent series:
$$
...
N.B., in the book "Non-Perturbative Field Theory" there is no minus sign in field commutator nor $2\pi$ in creation and annihilation operators commutator, but they seem necessary by calculating by hand.

By substituting the mode expansion into Noether charges, one can get that energy-momentum operators are proportional to $a^\dagger(k)a(k)+a(k)a^\dagger(k)$, and their vacuum expectation values are proportional to $\delta(0)~L$, where $L$ is the extent of the space direction, which diverges for infinite Minkowski or Euclidean space. Therefore one defines a normal ordered operator as:
$$
...
#### Radial quantization
For a cylindrical geometry, it is convenient to write $z=e^{x^0+ix^1}$, where the space becomes the phase and the (exponent of the) time becomes the magnitude of a complex number. Then the Noether charge can be written as a contour integral,
$$
Q=\int~dx^1J_0(z^1)=\int d\theta~J_r(\theta)=\
\frac{1}{2\pi i}\oint[dz~J(z)+d\bar z~\bar j(\bar z)].
$$
Infinitesimal change of an operator due to a Noether charge is
$$
\delta{\cal O} = -i\epsilon[Q,{\cal O}],
$$
in this case,
$$
\delta_{\epsilon,\bar\epsilon}{\cal O}=\frac{1}{2\pi i}\oint
dz[J(z)\epsilon(z),{\cal O}(w,\bar w)]+
d\bar z[\bar J(\bar z)\bar\epsilon(\bar z),{\cal O}(w,\bar w)].
$$
*Different than explained in the book*, If one introduces the *radial ordering* (which is here the time ordering) to the above expression, then the first term in the commutator is the one, where $|z|>|w|$, so it is an integration over a contour that contains $w$. In the second term, $|w|>|z|$, so the integration is over a contour that does not include $w$. The difference (the commutator itself) is over a contour that just encircles $w$. This allows to write
$$
\delta_{\epsilon,\bar\epsilon}{\cal O}=
\frac{1}{2\pi i}\oint
dz~\epsilon(z){\cal R}(J(z){\cal O}(w,\bar w))+
d\bar z~\bar\epsilon(\bar z){\cal R}(\bar J(\bar z),{\cal O}(w,\bar w)).

$$
#### Operator Product Expansion
The idea is to write
$$
{\cal O}_i(x){\cal O}_j(y)=\sum_kc^k_{ij}(x-y){\cal O}_k(y),
$$
where the so-called **coefficient functions** $c^k_{ij}(x-y)$ are singular in the limit $x\rightarrow y$. 
...
$$
Naively calculating the algebra, one gets
$$
[L_m,L_n]=(m-n)L_{m+n},
$$
