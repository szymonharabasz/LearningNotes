...
###### In the case of the chiral symmetry
The symmetry group is a direct product, $G=SU(2)_R\otimes SU(2)_L$, so its element can be represented as $g=\{V_R,V_L\}$. The unbroken subgroup $H=SU(2)_V$, generated by the charges of the vector current, so it consists of elements such that $V_L=V_R\equiv V$. So, $h=\{V,V\}$, and:
$$
g=\{V_R,V_L\}=\{(V_RV_L^\dagger)V_L,V_L\}=
\{V_RV_L^\dagger,1\}\{V_L,V_L\}\equiv\gamma h
$$
with the element of $G/H$:
$$
\gamma=\{U,1\},~~~U=V_R\V_L^\dagger\equiv\exp(i\vec\xi\cdot\vec\tau).
$$
When it is done at every spacetime point, $U(x)$ and $\vec\xi(x)$ are again fields and $\vec\xi(x)$ are proportional to the Goldstone fields. Under an action of $g$, $\gamma$ must transform into a representative of some coset, $\gamma'$:
$$
g\gamma=\{V_R,V_L\}\{U,1\}=\{V_RU,V_L\}=\{V_RUV_L^\dagger,1\}\{V_L,V_L\}\equiv
\gamma'h.
$$
So the transformation rule is:
$$
U(x)\rightarrow U'(x)=V_RU(x)V^\dagger_L.
$$
Another decomposition is:
$$
g=\{V_R,V_L\}\equiv\{u,u^\dagger\}\{V,V\},~~~V\in SU(2)_V.
$$
The action of $g$ is
$$
g\{u,u^\dagger\}=\{V_R,V_L\}\{u,u^\dagger\}=
\{V_Ru,V_Lu^\dagger\}=\{V_RuV^\dagger,V_Lu^\dagger V^\dagger\}\{V,V\}.
$$
So the transformation rules for $u$ are:
$$
\begin{split}
u(x)\rightarrow & u'(x)=V_Ru(x)V^\dagger,\\
u(x)\rightarrow & u'(x)=Vu(x)V_L^\dagger.
\end{split}
$$
It follows a rule for $u^2$:
$$
u^2\rightarrow u'^2=V_Ru^2V^\dagger_L,
$$
